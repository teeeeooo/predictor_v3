...
외삽은 성능값을 과대평가할 수 있으므로 변경 시 golden 테스트뿐 아니라 bin별 trace의 source를 확인해야 한다. 특히 warm bin에서 COPPL 외삽이 발생하면 final SCOP 영향이 클 수 있다.
...
| final rounding | return values | 최종 출력은 현재 구현 기준 반올림을 따른다. |

## 8. 테스트 전략

| 변경 유형 | 필수 확인 |
| --- | --- |
| 문서만 변경 | 변경 파일이 docs/en14825 하위 Markdown으로 제한되었는지 확인 |
...
```text
AGENTS.md의 Lite 규칙과 docs/DOCS_GUIDELINES.md를 먼저 읽어라. EN14825 문서는 docs/en14825_scop_notes.md와 core/calculator_en14825.py를 Primary 기준으로 삼고, PDF는 Clause/Table/Equation 확인용으로만 사용하라. 문서 작업이면 docs/en14825/glossary.md, docs/en14825/notes.md, docs/en14825/design_notes.md, docs/en14825/dev_notes.md만 수정하라.
```
...
```text
AGENTS.md와 docs/en14825/notes.md를 먼저 읽어라. data/region_configs/en14825_scop.json의 Table 37 또는 Annex D 값을 변경할 때는 출처 메타데이터를 유지하고, temperature/hour 길이와 heating_bin_hours_total이 일치하는지 확인하라.
```
