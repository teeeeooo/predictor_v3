...
- ISO16358-2 HSPF common path는 `tests/test_iso16358_hspf_formula_micro.py` H-1a safe branch micro tests로 보호한다. H-1b Formula 50 frost branch는 micro test 후보이며, above-extended saturated behavior는 contract 확인 전 expected로 고정하지 않는다.
...
- [DONE] 단기 TODO: `data/usa_hspf2.json`을 `data/region_configs/usa_hspf2.json`으로 이동하고 테스트/문서 참조를 갱신한다.
...
- Track A H-1a safe branch micro tests 완료. H-1b extended/auxiliary contract, KS shared-formula oracle consistency gate, Track B AS/NZS Excel compatibility guard test를 분리 설계.
