...
| `data/usa_hspf2.json` | JSON | `data/usa_hspf2.json` | HSPF2 Region IV canonical bin table, test point schema, legacy alias를 담는다. 이번 문서 작업에서는 수정하지 않는다. |
| `canonical_hspf2_bin_tables.heating.region_iv` | JSON object | `data/usa_hspf2.json` | 현재 v3 경로가 사용하는 Region IV Table 16 canonical table이다. legacy `bin_data`와 혼동하지 않는다. |
| `fractional_bin_hours` | list 또는 number | `data/usa_hspf2.json`, return dict | Table 16의 분수 빈 시간이다. 계절 합산 전 `heating_load_hours`와 곱한다. |
| `heating_load_hours` | number | `data/usa_hspf2.json` | HLH 값이다. 현재 Region IV는 1701이며 absolute hours 계산에 사용된다. |
| `zero_load_temp_f` | number | `data/usa_hspf2.json` | `t_zl`에 해당한다. `BL(tj)` 계산에 사용한다. |
| `outdoor_design_temp_f` | number | `data/usa_hspf2.json` | `t_od`에 해당한다. `BL(tj)` 계산에 사용한다. |
| `variable_capacity_slope_factor` | number | `data/usa_hspf2.json` | `C_vs`에 해당한다. `BL(tj)` 계산에 사용한다. |
