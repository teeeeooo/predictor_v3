...
3. Repo 다음 순서는 다음 sequence로 둔다:
   ...
   4v. **Xfail retirement audit / ISO pure-route obsolete xfail retirement** (122, 123, 124 참고). PyQt fatal-abort 4개 파일 제외 full-ish baseline은 **568 passed, 1 skipped, 23 xfailed**에서 ISO pure-route Formula 45/49/47/50 obsolete experiment xfail 4개 제거 후 **568 passed, 1 skipped, 19 xfailed**로 정리되었다. `tests/test_iso16358_hspf_pure_iso_track_a.py`에는 fixture identity / workbook-reference guard / cycling / Formula 44·48 min-half / saturated auxiliary smoke만 남겼다. 남은 xfail은 `tests/_legacy/` 17개 active diagnostic/reference와 AS/NZS case3 external reference/full row-data prerequisite 2개다. 다음 recommended action은 (1) `tests/_legacy` diagnostic xfail reason/owner cleanup, (2) AS/NZS case3 external reference compatibility decision, (3) Windows host 확보 시 PyInstaller size measurement. PyQt fatal-abort 4개 파일은 별도 environment handling slice로 유지한다.
