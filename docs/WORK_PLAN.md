...
3. Repo 다음 순서는 다음 sequence로 둔다:
   ...
   4v. **Xfail retirement audit** (122 참고). PyQt fatal-abort 4개 파일 제외 full-ish baseline은 **568 passed, 1 skipped, 23 xfailed**로 재확인했다. xfail 출처는 `tests/_legacy/` 17개와 non-legacy 6개 (`tests/test_iso16358_hspf_pure_iso_track_a.py` 4개, AS/NZS case3 packet/component-row tests 2개) 로 수량이 맞는다. targeted `--runxfail` 기준으로 즉시 stale marker removal 후보는 없음. 후속 cleanup은 (a) `tests/_legacy` diagnostic xfail reason/owner cleanup, (b) external reference compatibility decision, (c) ISO pure-route calculator-specific expected/fixture/core mismatch audit, (d) AS/NZS full row-data prerequisite audit, (e) PyQt environment-dependent fatal-abort handling으로 분리한다. 추천 first cleanup은 **legacy diagnostic xfail reason/owner cleanup**이며, legacy cleanup C2보다 xfail retirement audit/cleanup을 먼저 수행하는 것이 현재 test debt 축소에 더 직접적이다. 단, marker 제거 / expected / fixture / core 변경은 별도 slice로 분리한다.
