...
3. Repo 다음 순서는 다음 sequence로 둔다:
   1. Calculator UI/UX audit against new SSOT (`docs/ui_ux/`) — 계산기 UI surface가 새 UI/UX SSOT / table UX contract / PyQt adapter와 정렬되는지 audit. (이 audit phase에서는 코드 변경은 포함하지 않는다.)
