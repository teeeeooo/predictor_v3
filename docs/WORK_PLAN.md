...
3. Repo 다음 순서는 다음 sequence로 둔다:
   ...
   4v. **Xfail retirement audit** (122 참고, 123 lifecycle summary에서 최신 판단 보정). PyQt fatal-abort 4개 파일 제외 full-ish baseline은 **568 passed, 1 skipped, 23 xfailed**로 재확인했다. xfail 출처는 `tests/_legacy/` 17개와 non-legacy 6개 (`tests/test_iso16358_hspf_pure_iso_track_a.py` 4개, AS/NZS case3 packet/component-row tests 2개) 로 수량이 맞는다. targeted `--runxfail` 기준으로 즉시 stale marker-only removal 후보는 없음. 단, ISO16358-2 HSPF official exact golden이 16/16 pass 상태이므로 ISO pure-route Formula 45/49/47/50 xfail 4개는 active known mismatch가 아니라 **obsolete experiment / retirement candidate**로 우선 재검토한다. 다음 recommended action은 (1) ISO pure-route Formula 45/49/47/50 obsolete test retirement audit/removal slice, (2) `tests/_legacy` diagnostic xfail reason/owner cleanup, (3) AS/NZS case3 external reference compatibility decision, (4) Windows host 확보 시 PyInstaller size measurement. PyQt fatal-abort 4개 파일은 별도 environment handling slice로 유지한다.
