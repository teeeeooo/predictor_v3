...
| `2026-07-01-arc13-5-feature-catalog-editor-design-gate.md` | active reference | Design Arc 13.5 around an `app_train.py` Feature Catalog table editor workflow instead of default direct CSV editing. | `docs/designs/2026-07-01-arc13-5-feature-catalog-editor-design-gate.md`, `docs/WORK_PLAN.md`, `result_reports/legacy/summaries/633_summary-arc13-feature-catalog-closeout.md` | Implementing Arc 13.5 viewer, editor, export, or save slices. |
...
| `2026-07-05-arc14b4-legacy-mapping-csv-to-json-rule-reconstruction-audit.md` | active reference | Legacy single CSV to `mapping.json` restoration is not recoverable enough for implementation; proceed with UI CRUD workflow design, Import disabled, and Export as read-only snapshot. | this audit, `docs/WORK_PLAN.md`, current `core.mapping` runtime owners | Planning Data Mapping CRUD, import removal/compatibility, or read-only export snapshot behavior. |
| `2026-07-05-arc14b5-data-mapping-ui-crud-workflow-design.md` | active reference | Define the user-facing Data Mapping CRUD workflow as Predict mapping group editing, with ODU Cond Specs generating internal cascade/spec sections, Import excluded, and Export read-only. | this design, `docs/WORK_PLAN.md`, current Data Mapping service/controller/UI owners | Implementing Arc 14B-5B editor draft projection, Data Mapping CRUD, Save, Reload, or Export snapshot slices. |
| `2026-07-05-arc14c-runtime-cascade-integration.md` | active reference | Connect Data Mapping Manager runtime sections to Predict dropdown/autofill cascade while preserving `mapping.json` as SSOT and avoiding fallback options. | this design, `core/mapping/autofill.py`, `apps/predict/adapters/dropdown_option_adapter.py`, `apps/predict/controllers/input_edit_controller.py` | Implementing or reviewing Arc 14C runtime cascade, missing mapping behavior, or ODU Cond Specs dropdown/autofill integration. |
| `2026-07-06-arc14d-data-mapping-xlsx-export-ui-polish.md` | active reference | Record Arc 14D-R JSON/XLSX read-only export contract, `openpyxl` writer use, no import contract, Data Mapping wording polish, and invalid mapping badge behavior. | this audit, `docs/development/dependencies.md`, `requirements/excel.txt`, `requirements/ml_runtime.txt`, `core/mapping/editor_export.py`, `apps/train/services/data_mapping_service.py`, `apps/train/controllers/data_mapping_controller.py`, `apps/train/ui/data_mapping_panel.py`, `apps/predict/ui/status_widgets.py`, `apps/predict/ui/workspace.py` | Reviewing Data Mapping snapshot export behavior, Export dialog polish, or invalid mapping badge wording. |
| `2026-07-06-arc15-unified-data-definition-manager-foundation.md` | active reference | Promote Predict Schema v2 into the primary Data Definition workflow while keeping `features.csv` as staged ML projection/compatibility output and `mapping.json` as value SSOT. | this design, `config/predict/schema.csv`, `config/ml/features.csv`, `docs/WORK_PLAN.md` | Implementing Arc 15A, Add Feature workflow, Feature Catalog owner switch, or Data Mapping dynamic attribute requirement work. |
