...
- canonical domain key는 그대로 유지하되 user-visible point header는 M HSPF에서 M1과 동일하게 `H2v`, `H1N(STD)`를 사용하고 M SEER의 `EV`는 `Ev`로 표시한다.
- M SEER result의 seasonal aggregate 표시는 `CSTL [Btu/h]`, `CSEC [W]`로 통일한다. M HSPF aggregate 표시는 `Heating Load [Btu/h]`, `Compressor Input [W]`, `Auxiliary Input [W]`를 사용한다. 내부 result key와 계산 의미는 변경하지 않는다.
