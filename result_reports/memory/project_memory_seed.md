...
entries:
  ...
  - type: decision
    ...
    content: Phase 5A is closed and `docs/designs/2026-07-22-train-model-lifecycle-agent-assisted-experiment-design.md` remains authoritative. The independent audit of Phase 5B exact head `21b98eb38239e0100be3c3700744c69e2fdc11fe` returned PASS, and PR #28 was squash-merged to `main` as `eca6addd38745dadca3b0e4f19cc259090d50e23`. Phase 5B is complete with immutable Candidate publication, explicit revision-guarded Active promotion and rollback, recovery, and Predict startup resolution. Training success never activates a model automatically. Phase 5C is implemented on PR #30 with Qt-free `training_result.v1`, Core-owned target R²/MAE/RMSE and fold stability, fair Active/Bootstrap baseline handling, RFECV, target-local XGBoost gain importance, Optuna, preprocessing/data-quality, and Candidate-owned JSON/CSV/XLSX under manifest v2. Its first independent L4 audit returned FAIL. The bounded repair makes Candidate read/promotion share lifecycle-owned exact artifact/schema validation, projects baseline/delta/unavailable/target-failure decisions across JSON/CSV/XLSX, injects filesystem/report/hash adapters at composition, corrects preprocessing stage semantics, and adds a bounded real production multi-target path through Candidate publication. Legacy manifest v1 stays read-only; partial/failed/cancelled evidence remains non-promotable outside Candidate publication; Active remains unchanged. Phase 5C awaits exact-head independent L4 re-audit; Phase 5D, CLI, Campaign, and the Agent-assisted Experiment Loop remain unstarted. Earlier FAIL, repair, and validation records retain their historical meaning; Windows-native and packaging validation remain unrun.
    keywords:
      - predictor_v3
      - Train/Admin
      - Phase 5A
    ...
    assertionStatus: verified
    source: docs/designs/2026-07-22-train-model-lifecycle-agent-assisted-experiment-design.md; result_reports/records/2026-07/2026-07-25-train-admin-phase5c-result-integrity-audit-repair.md; docs/WORK_PLAN.md
