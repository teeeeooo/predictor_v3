...
entries:
  ...
  - type: decision
    ...
    content: Phase 5A is closed and `docs/designs/2026-07-22-train-model-lifecycle-agent-assisted-experiment-design.md` remains authoritative. The independent audit of Phase 5B exact head `21b98eb38239e0100be3c3700744c69e2fdc11fe` returned PASS, and PR #28 was squash-merged to `main` as `eca6addd38745dadca3b0e4f19cc259090d50e23`. Phase 5B is complete with immutable Candidate publication, explicit revision-guarded Active promotion and rollback, recovery, and Predict startup resolution. Training success never activates a model automatically. Phase 5C is implemented on Draft PR #30 with Qt-free `training_result.v1`, Core-owned metrics/analysis, fair baseline handling, and Candidate-owned JSON/CSV/XLSX under manifest v2. Its first, second, and third independent L4 audits returned FAIL. The first two blocker sets remain repaired: read/promotion share lifecycle-owned exact artifact/schema validation, decision fields project across artifacts, file adapters are injected at composition, a bounded production multi-target path runs, artifact identities and raw JSON types are strict, original Core evidence is hash-validated on success, and publication failures preserve target evidence with a truthful minimal fallback. The third audit's sole blocker is repaired by normalizing malformed persisted payloads to an explicit artifact-contract error while allowing unexpected validator/parser/helper `TypeError` and `AttributeError` to propagate through Candidate read, publication validation, and promotion without corruption relabeling. Legacy manifest v1 stays read-only; failed evidence remains non-promotable outside Candidate publication; Active remains unchanged. Phase 5C awaits another final exact-head independent L4 re-audit; Phase 5D, CLI, Campaign, and the Agent-assisted Experiment Loop remain unstarted. Earlier FAIL, repair, and validation records retain their historical meaning; Windows-native and packaging validation remain unrun.
    keywords:
      - predictor_v3
      - Train/Admin
      - Phase 5A
    ...
    assertionStatus: verified
    source: docs/designs/2026-07-22-train-model-lifecycle-agent-assisted-experiment-design.md; result_reports/records/2026-07/2026-07-25-train-admin-phase5c-programmer-error-boundary-repair.md; docs/WORK_PLAN.md
