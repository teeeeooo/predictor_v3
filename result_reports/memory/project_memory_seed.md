...
entries:
  ...
  - type: decision
    ...
    content: Phase 5A is closed and `docs/designs/2026-07-22-train-model-lifecycle-agent-assisted-experiment-design.md` remains authoritative. Phase 5B through Phase 5G are independently audited, merged, and complete; Phase 5G final audit accepted exact head `3344be1237f56752f8fcb607074152c53ea75c52`, required run `30203030680` succeeded, and PR #34 was squash-merged as `cb9183353dd6492dff07012c4276e4beb2b582b8`. Phase 5H remains on one Lane C Draft branch pending independent exact-head re-audit. Preserve its first audit `FAIL` at `009e4d3d668df2fc3df13a06aa22a8991ec67299` and historical successful run `30280886312` as non-acceptance evidence. The bounded repair binds snapshot materialization and complete captured artifacts to selected-run evidence, computes locked-test seal identity from data/membership/Targets/split/schema/evaluation/source hashes and performs immutable sealed-data evaluation, requires trusted opaque interactive authority plus one final decision, blocks orphan confirmation Candidates, makes terminal replay and migration/retention previews byte-invariant, and inventories true lifecycle source references while missing export/lease state remains fail-closed. Recommendation/confirmation cannot grant agent approval or bypass generic promotion policy. No production confirmation/promotion, deployment change, migration apply, delete apply, merge, or Worker audit-PASS authority is included.
    keywords:
      - predictor_v3
      - Train/Admin
      - Phase 5A
