...
entries:
  ...
  - type: procedure
    ...
    content: Exact next gate is a bounded Predict Result Review repair for pinned-anchor/right-table header-first-row alignment and the unavailable cooling/heating ReviewSourceValue tooltip AttributeError. After exact-head Gate/merge/Close, resume native Windows 11 Enterprise source-runtime acceptance by preserving/inspecting affected state when available and then clean-state Train/Predict/Experiment/Calculator/Deployment Export, with real Train completion through Candidate publication. Then execute the Data Definition/Data Mapping onboarding workstream. Standard Request Product/Owner Decision and Multi-point Predict → Calculate remain held until native Windows acceptance closes; Result Review XLSX remains DEFER.
