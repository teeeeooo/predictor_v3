...
entries:
  ...
  - type: decision
    ...
    content: Phase 5A is closed with PASS and `docs/designs/2026-07-22-train-model-lifecycle-agent-assisted-experiment-design.md` is authoritative. Phase 5B remains under independent audit on Draft PR #28. Audit of head `7ac586a7610b02cfd1cb4b47d72bec7a70481a23` found that top-level Active identity/timestamp could contradict latest history and committed cleanup recovery was bound only to revision. The bounded repair validates append-only history from revision 1, path-safe Candidate identities, and exact top-level/latest identity, revision, and activation-time agreement; committed markers now carry intended Candidate identity and corrupt or mismatched recovery preserves marker/backup evidence. Earlier descriptor-relative locking, rollback-first indeterminate recovery, mandatory revision guards, fail-closed legacy handling, explicit Qt fixture ownership, narrow resolver errors, and QProcess cleanup remain preserved. Local and remote validation are repair evidence for a new exact-head re-audit, not final approval. Phase 5C has not started; Train/Model UI/UX remains primary and CLI/campaign/agent-loop work stays later.
    keywords:
      - predictor_v3
      - Train/Admin
      - Phase 5A
