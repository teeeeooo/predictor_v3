...
entries:
  ...
  - type: decision
    ...
    content: Phase 5A is closed with PASS and `docs/designs/2026-07-22-train-model-lifecycle-agent-assisted-experiment-design.md` is authoritative. Phase 5B remains under independent audit on Draft PR #28. Audit of second-repair head `f944cbc6d40a36db94a599462bb2cdcc21ce2265` found that cleanup after a durable Active commit could restore or permanently block the wrong revision, the global autouse QApplication fixture broke non-Qt CI, and Active startup resolution hid programmer errors. The bounded repair records committed Active state before cleanup, reconciles that state forward without duplicate revision/history, keeps QApplication ownership explicit to Qt/QProcess tests, and narrows controlled resolver failures to lifecycle/filesystem errors. Descriptor-relative locking, rollback-first indeterminate recovery, mandatory revision guards, fail-closed legacy handling, and QProcess terminal cleanup remain preserved. Local and remote validation are implementation evidence for a new exact-head re-audit, not final approval. Phase 5C has not started; Train/Model UI/UX remains primary and CLI/campaign/agent-loop work stays later.
    keywords:
      - predictor_v3
      - Train/Admin
      - Phase 5A
