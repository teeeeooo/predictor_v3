...
## Workstream Summary
Reports 023~032은 하나의 큰 흐름을 다룬다. (1) calculator architecture를 ISO 16358 / KS C 9306 / AS/NZS workbook oracle 3-module boundary로 reset하고 문서를 정정한다. (2) 그 boundary대로 KS C 9306 코드를 `core/calculator_iso16358.py`에서 분리해 `core/calculator_ks_c9306.py` (`KSC9306Calculator`)로 옮기고 CSPF/HSPF public entry를 마련한다. (3) profile manifest에 KS C 9306 두 profile을 등록하고 snapshot test를 정렬한다. (4) profile resolver 결과를 calculator 인스턴스로 잇는 thin dispatch helper(`core/calculator_dispatcher.py`)를 추가해 Calculator UI 연결 전 단계의 foundation을 완성한다. 별도로 029에서 모든 agent 작업의 터미널 출력에 `modified:` 한 줄을 표준 형식으로 추가하는 process 변경이 함께 이루어졌다.
...
## Validation Summary
- 본 workstream 시작 직전 baseline (보고 022 push 직후): `258 passed, 16 failed, 13 xfailed` (16 failures 모두 ISO common HSPF formula 47/50, pure ISO track A, case 3 Excel BM/BO/CD/Y_MIN_Y_EXTD trace, half-to-full / tiny-bin accumulation, formula50 frost 등 pre-existing known mismatch).
...
- 두 갱신 모두 covered report 전문을 복사하지 않고 결정 수준의 짧은 요약만 남긴다.
