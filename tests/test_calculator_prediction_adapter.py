...
@pytest.mark.parametrize("source", ["manual_candidate", "ml_prediction", "fixture"])
def test_envelope_accepts_all_design_source_values(source):
    envelope = build_predicted_points_envelope(
        "ahri_usa_seer2",
        _full_predicted_points(),
        source=source,
...
def test_envelope_rejects_unsupported_capacity_unit():
    bad = _full_predicted_points()
    bad["A_Full"]["capacity_unit"] = "kW"
    with pytest.raises(ValueError, match="capacity_unit must be 'Btu/h'"):
        build_predicted_points_envelope("ahri_usa_seer2", bad)


def test_envelope_rejects_unsupported_power_unit():
    bad = _full_predicted_points()
    bad["A_Full"]["power_unit"] = "kW"
    with pytest.raises(ValueError, match="power_unit must be 'W'"):
        build_predicted_points_envelope("ahri_usa_seer2", bad)
