...
def format_seer2_bin_details(
    ...
    """Normalize product-specific core diagnostics into the shared UI schema."""
    formatted = []
    for row in rows:
        q_low = _first(row, "q_Low", "q_low")
        p_low = _first(row, "p_low")
        q_full = _first(row, "q_Full", "q_full")
        p_full = _first(row, "p_full")
        case = _first(row, "operating_case", "case")
        formatted.append(
            {
                "bin_no": optional_fixed_number(_first(row, "bin", "bin_no"), 0),
                "tj": optional_fixed_number(row.get("temp_F"), 1),
                "operating_case": optional_text(case),
                "building_load": optional_fixed_number(_first(row, "BL", "building_load"), 1),
                ...
                "eer_full": optional_fixed_number(
                    ...
                    else (float(q_full) / float(p_full) if q_full and p_full else None),
                    3,
                ),
