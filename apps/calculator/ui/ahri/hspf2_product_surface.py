...
class AhriHspf2ProductSurface:
    ...
    def __init__(
        ...
        *,
        ...
        self._controllers = tuple(
            TkTableController(table) for table in self._tables
        )
    ...
    def _build_control_bar(self, snapshot: Mapping[str, object]) -> None:
        ...
        self.h2_low_var = tk.BooleanVar(
            master=self.frame,
            value=bool(flags.get("h2_low", h2_low_default)),
        )
        ...
        for variable in (
            ...
            variable.trace_add(
                "write", lambda *_args: self._on_state_changed()
            )
    ...
    def _build_numeric_table(
        ...
        if self.product == "dual_stage":
            columns.append(
                ("low_stage_lockout_temp_c", "Low Lockout [°C]")
            )
        table = MetricInputTable(
            ...
            rows=(("value", "Value"),),
            editable_cells={
                ("value", key): key for key, _label in columns
            },
    ...
    def _build_stage_table(
        ...
        for point in points:
            table.static_cell_labels[("condition_temp", point)].configure(
                text=(
                    f"Heating / {AHRI_HSPF2_TEMPERATURES_C[point]:.1f} °C"
                )
            )
    ...
    def _apply_states(self) -> None:
        ...
        if self.product == "dual_stage":
            enabled_numeric["low_stage_lockout_temp_c"] = (
                self.low_lockout_var.get()
            )
        numeric_readonly = {
            ("value", key)
