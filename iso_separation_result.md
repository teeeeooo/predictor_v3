...
- Existing mixed ISO implementation was archived to `core/_legacy/calculator_iso16358_legacy.py`.
...
- `core/calculator_asnzs_hspf_excel.py` owns AS/NZS Excel compatibility snapshot calculation under `ASNZS_EXCEL_COMPAT` only. Current local workbook HSPF and CSPF snapshot rows are covered; historical case3 full-dump parity remains Z-phase.
...
- ISO HSPF common branch routing now calls Formula 44/45/47/48/49/50 helpers for the active branch paths, including frost extended fallback from `2_ext`.

## Verification Summary

- Step 5 targeted checks: `30 passed` for profile/dispatcher tests.
- AS/NZS compatibility checks after completion: included in full suite and targeted workbook checks.
- ISO HSPF targeted completion checks: `81 passed, 21 xfailed`.
- Full suite after completion: `288 passed, 23 xfailed`.
- The former 16-failure group was split: active ISO formula/validation failures were fixed, and archived workbook diagnostic expectations were marked as explicit xfail.
