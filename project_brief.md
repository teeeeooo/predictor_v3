...
## 1. 현재 상태 및 마일스톤
- **테스트 및 검증:** Validation smoke/golden 안정화 및 KS oracle cycling consistency(H-2b) 검증이 완료되었습니다. ISO HSPF active formula/validation failures는 해소했고, historical workbook diagnostic 기대값은 explicit xfail로 격리했습니다.
...
- **Calculator UI / ML adapter 경계:** `app_calculator.py` / `ui/calc_window.py`는 PyQt offscreen launch smoke로 보호하고, AHRI SEER2 selector는 profile resolver 기반입니다. ML / inverse-search 복귀 전 calculator result envelope / ML adapter boundary는 `docs/designs/2026-05-17-calculator-result-envelope-ml-adapter.md`를 기준으로 합니다.
